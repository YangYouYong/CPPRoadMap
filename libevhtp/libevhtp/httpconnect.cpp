...
//  libevhtp
//
//  Created by yangyouyong on 2018/7/11.
...
//    }
    ...
    const char *uri = "127.0.0.1";
    {
        VERIFY(proxy = evhttp_uri_parse(uri));
        VERIFY(evhttp_uri_get_host(proxy));
        VERIFY(evhttp_uri_get_port(proxy) > 0);
    }
    const char *port = "9900";
    {
        host_port = evhttp_uri_parse(port);
    ...
    {
        location = evhttp_uri_parse(port);
